...
- `bridge.sh` — Admin CLI dispatcher, installed as `/usr/local/bin/bridge` (symlink). Subcommands (`status`, `logs`, `restart`, `update-now`, `os-update`, `node-upgrade`, `cleanup-legacy`, `backup-now`, `sync-players`, `wifi-scan`, `ship-logs`, `version`, `wifi`, `password`, `reboot`, `help`) are thin wrappers over the scripts/units. App subcommands use `systemctl`/`journalctl` on `bridge-box-app.service`; system ones use sudo. Add new common tasks here rather than making users memorise long paths.
