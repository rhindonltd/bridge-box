...
- `bridge-box-wifi-apply.path` / `bridge-box-wifi-apply.service` — **Instant apply of `wifi.json`.** The `.path` unit watches `/home/bridgebox/wifi.json` (`PathChanged=`); when it changes, the `.service` (oneshot, runs as `bridgebox`) invokes `sudo -n /usr/local/bridgebox/bin/wifi-ctl.sh connect`, bringing the **client radio** onto the new network with no reboot. Client-only (the hotspot, a separate radio, is never touched); best-effort (a failure just leaves the box on the old link / hotspot). This is what makes the app's "write `wifi.json` to switch networks" take effect immediately. The app must write the file **atomically** (temp + rename) so the watcher never fires on a partial write.
...
- `/etc/systemd/system/bridge-box-wifi-apply.{path,service}` (the `.path` watches `wifi.json` and applies it to the client radio on change — no reboot)
...
- **App ↔ provisioning networking boundary.** The scorer app (as `bridgebox`) must NOT drive NetworkManager directly (it gets "Not authorized to control networking"). Instead: the app does read-only `nmcli` (diagnostics, `nmcli -t -f ACTIVE,SSID dev wifi`, `connection show`) and `command -v nmcli` **directly** (reads need no auth); for privileged operations it calls `sudo -n /usr/local/bridgebox/bin/wifi-ctl.sh <scan|test-connect|test-cleanup>` (these act on the client radio only); and to *commit* a chosen network it just **writes `wifi.json`** (a plain file write — a systemd `.path` unit, `bridge-box-wifi-apply.path`, watches the file and runs `bridge-box-wifi-apply.service` → `wifi-ctl.sh connect` to bring the client radio onto it **immediately, no reboot**; boot still also applies it via `bridge-box-root.sh`). The app **must write `wifi.json` atomically** (write a temp file, then `rename()` over `wifi.json`) so the watcher never sees a half-written file. The app never activates the real hotspot/client connections itself. Credential testing uses the throwaway `bridge-box-wifi-test` profile via the helper. This keeps radio ownership with provisioning and `bridgebox` unprivileged for general NM.
