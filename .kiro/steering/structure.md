...
- **Single WiFi radio: can't be a hotspot AND join a client network at once.** Before a client connect, `bb_connect_wifi` disables the hotspot profile's autoconnect (it has high priority and would re-steal the radio) and brings the hotspot down, then rescans. `bb_return_to_hotspot` re-enables autoconnect and brings the hotspot back. Symptom if this is broken: client connect fails with "No network with SSID … found" while the hotspot is up. Keep the down-then-up (and autoconnect toggle) around any client-mode switch.
