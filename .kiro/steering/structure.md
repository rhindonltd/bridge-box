...
- **Services that use PM2 must set `HOME` (and `PM2_HOME`).** systemd doesn't set `$HOME` from `User=`, so PM2 would use a different/empty home than an interactive `bridgebox` shell — the app would appear "started" but not be visible to `pm2 status` and not actually serve. All bridgebox+PM2 units (`update`, `build`, `healthcheck`) set `Environment=HOME=/home/bridgebox` and `PM2_HOME=/home/bridgebox/.pm2`. Keep this on any new PM2-using unit. `start_app` also verifies the process registered rather than assuming success.
