...
- **The app is started by launching its entrypoint directly, not via `npm start`.** `npm start` under systemd is fragile (npm/tsx PATH resolution, and the `exec` in the app's start script makes PM2 lose the process). `start_app` runs `tsx` by absolute path (`<release>/node_modules/.bin/tsx ... server.ts`) with `--interpreter none`. This still depends on `tsx`; the app repo is being changed to compile `server.ts` → `dist/server.js` (see `scorer-startup-spec.md`), after which this should become plain `node dist/server.js`. `start_app` verifies the process is `online` (not just that `pm2 start` was issued).
